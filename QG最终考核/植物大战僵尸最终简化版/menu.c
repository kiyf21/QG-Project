...
static void Show_GameInfo(void)
{
	if(page==0)
	{
		...
		LCD12864_Show_String(0,3,"按S10 向下一页");
	}
	else if(page==1)
	{
		LCD12864_Clear();
		LCD12864_Show_String(2,0,"按键介绍：");
		LCD12864_Show_String(0,1,"上下左右确认返回");
		LCD12864_Show_String(1,2,"2 10 5 7 6 6");
		LCD12864_Show_String(0,3,"游戏中暂停为S16");
	}
	else if(page==2)
	{
		LCD12864_Clear();
		LCD12864_Show_String(2,0,"放置植物：");
		LCD12864_Show_String(0,1,"选择地块后确认");
		LCD12864_Show_String(0,2,"左右选豌豆或土豆");
		LCD12864_Show_String(1,3,"按确认放置");
	}
}
    ...
    while(1)
    {
		if(now_state != last_state)
        {
            ...
            switch(now_state)
            {
				...
				case GAME_SETTLEMENT:Show_SettleMent();  break;
				case SAVE_CHECK:     Show_SaveScore();  break;
            }
        }
        key = MatrixKey();
        if(key == 0) continue;
        switch(now_state)
        {
			...
			case GAME_SETTLEMENT:SettleMent_Key(key);  break;
			case SAVE_CHECK:     GameInfo_Key(key);  break;
