...
void LCD12864_Show_ImageTry (unsigned char code *DData);
...
void LCD12864_Show_ImageTry1 (unsigned char code *DData)
{
	...
	for(x=13;x<29;x++){				//32行
		LCD12864_Write_Cmd((0x80+x),1);//列地址
		LCD12864_Write_Cmd((0x80+0),1);	//行地址，下半屏，即第三行地址0X88	
		for(y=0;y<2;y++)
		{
			LCD12864_Write_Data(DData[tmp+y]);//读取数据写入LCD
		}
...
void LCD12864_Show_ImageTry3 (unsigned char code *DData)
{
	...
	for(x=15;x<31;x++){				//32行
		LCD12864_Write_Cmd((0x80+x),1);//列地址
		LCD12864_Write_Cmd((0x80+8),1);	//行地址，下半屏，即第三行地址0X88	
		for(y=0;y<2;y++)
		{
			LCD12864_Write_Data(DData[tmp+y]);//读取数据写入LCD
		}
...
void LCD12864_Show_ImageTry11 (unsigned char code *DData)
{
	...
	for(x=13;x<29;x++){				//32行
		LCD12864_Write_Cmd((0x80+x),1);//列地址
		LCD12864_Write_Cmd((0x80+1),1);	//行地址，下半屏，即第三行地址0X88	
		for(y=0;y<2;y++)
		{
			LCD12864_Write_Data(DData[tmp+y]);//读取数据写入LCD
		}
...
void LCD12864_Show_ImageTry22 (unsigned char code *DData)
{
	...
	{		
		if(i==0)
		{
			...
			{				
				LCD12864_Write_Cmd((0x80+x),1);//列地址
				LCD12864_Write_Cmd((0x80+i+1),1);	//行地址，下半屏，即第三行地址0X88	
				for(y=0;y<2;y++)
				{
					LCD12864_Write_Data(DData[tmp+y]);//读取数据写入LCD
				}
...
void LCD12864_Show_ImageTry33 (unsigned char code *DData)
{
	...
	for(x=15;x<31;x++){				//32行
		LCD12864_Write_Cmd((0x80+x),1);//列地址
		LCD12864_Write_Cmd((0x80+9),1);	//行地址，下半屏，即第三行地址0X88	
		for(y=0;y<2;y++)
		{
			LCD12864_Write_Data(DData[tmp+y]);//读取数据写入LCD
		}
...
//1bullet1
void LCD12864_Show_ImageBullet1 (unsigned char code *DData,unsigned char i)
{
	unsigned char x,y;
	unsigned int tmp=0;
...
void LCD12864_Show_ImageZombie1 (unsigned char code *DData,unsigned char i)
{
	...
	for(x=13;x<29;x++){				//32行
		LCD12864_Write_Cmd((0x80+x),1);//列地址
		LCD12864_Write_Cmd((0x80+7-i),1);	//行地址	
		for(y=0;y<2;y++)
		{
			LCD12864_Write_Data(DData[tmp+y]);//读取数据写入LCD
		}
...
//	{
//		for(y=SY;Y
//    LCD12864_Write_Cmd(0x30, 1);  // 恢复基本指令模式
//}


//// 修复版：LCD12864 通用图片显示（支持任意尺寸，完整显示128×64）
//// x   : 起始列 0~15
//// y   : 起始行 0~63
//// w   : 宽度（列数 1~16）
//// h   : 高度（行数 1~64）
//// dat : 图片数组
//void lcd_draw_image(unsigned char x, unsigned char y, unsigned char w, unsigned char h, unsigned char code *dat)
//{
//    unsigned int idx = 0;
//    unsigned char i, j;
//    unsigned char draw_w, draw_h;

//    // 越界保护
//    if(x >= 16 || y >= 64) return;
//    draw_w = (x + w > 16) ? (16 - x) : w;
//    draw_h = (y + h > 64) ? (64 - y) : h;
...
//    for(j = 0; j < draw_h; j++)
//    {
//        unsigned char cur_y = y + j;  // 当前行 0~63
//        unsigned char y_addr;         // 硬件列地址（关键）
...
//        lcd12864_write_cmd(0x80 + cur_y);   // 写行地址（0~63）
//        lcd12864_write_cmd(0x80 + x + y_addr);  // 写列地址：x(0~15) + 半屏基址(0/8)

//        // 写入一行图像数据
//        for(i = 0; i < draw_w; i++)
//        {
//            lcd12864_write_data(dat[idx++]);
//        }
//    }
